...
const CommonTable = ({
  ...
  const onBlur = (event, value , row) => {
    ...
    console.log("w",event.target.name)
    temp[row?.PRIMARY_ACCOUNT][event.target.name] = event.target.value;    
  ...
  return (
    <>
      ...
      {(pageName != "stage") &&
        <TableToolbar selected={selected} handledelete={handleDelete} edithandle={handleEdit} seteditRows={seteditRows} setUpdateRow={setUpdateRow} setSelected={setSelected} editRows={editRows} />
        } 
